...

{
  ...
  programs.zsh = {
    ...
    shellAliases = {
      futaba = "sudo nixos-rebuild switch --flake /home/olivernix/flakes/#laptop --max-jobs 2";
  ...
  users.users.olivernix = {
    ...
    extraGroups = [ "networkmanager" "wheel" "video" "docker" "plugdev" "audio" "openrazer" "ydotool" "input" "dialout" "tty" ];
    packages = with pkgs; [ ];
    shell = pkgs.zsh;
  };
}
